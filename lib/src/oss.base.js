...
class OSSBase {
    ...
    getUploadSgin() {
        const policyText = {
            'expiration': `${moment().add(1, 'hours').format('YYYY-MM-DDTHH:mm:ss')}.000Z`,
            'conditions': [
                ['content-length-range', 0, 50048576000] // 设置上传文件的大小限制
            ]
        };
        const policyBase64 = Buffer.from((JSON.stringify(policyText))).toString('base64');
        const uploadSignature = crypto_1.createHmac('sha1', this.options.client.accessKeySecret).update(policyBase64).digest('base64');
        const params = {
            policy: policyBase64,
